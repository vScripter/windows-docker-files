...
if not [%ipPolicy%]==[""] (
  if "%ipPolicy%" == "static" (
    if ["%gateway%"]==["gateway=[empty] gwmetric=default"] (
		netsh interface ip delete address %connectionName% gateway=all
		set gateway=
	) 
...
if not [%dnsPolicy%]==[""] (
  ...
  if "%dnsPolicy%" == "static" (
    ...
    if ["%dns1%"]==["[empty]"] (
		...
		if not [%dns1%] == [""] (
		   netsh interface ip delete dns %connectionName% all >null
		   netsh interface ip add dns %connectionName% %dns1% validate=no
		)
	    
		if not [%dns2%] == [""] (
		  netsh interface ip add dns %connectionName% %dns2% index=2 validate=no
		)
    )
  ) else (
