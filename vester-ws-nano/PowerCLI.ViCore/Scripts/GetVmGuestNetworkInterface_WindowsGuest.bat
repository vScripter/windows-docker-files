...
for /F "tokens=1,2 delims=:" %%A in (%output_file%) do (
  ...
  if not [%%B]==[] (
    ...
    rem IPAddress
    echo %%A | find /I "IP Address" >nul
    if !errorlevel!==0 (
      set IPAddress=IPAddress=!value!;
    )
    ...
    if !errorlevel!==0 (
		...
		set InterfaceIndex=RouteInterfaceId=!InterfaceIndex!;
        set Mac=Mac=!value!;
    )
    ...
    echo %%A | find /I "Ethernet adapter " >nul
    ...
    if !errorlevel!==0 (
      ...
      netsh interface ip show dns name="!tempName!" | find /I "dhcp" >nul
      if !errorlevel!==0 (
      ...
      netsh interface ip show wins name="!tempName!" | find /I "dhcp" >nul
      if !errorlevel!==0 (
